...
export default function Collection({ onAdd }: CollectionProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const headOps = headItems.map(el =>
        el.style.opacity ? parseFloat(el.style.opacity) : 1,
      );
  ...
  return (
    ...
    >
      <Grain />
      <div
        className="relative px-4 sm:px-24 py-20 sm:py-28"
        style={{ zIndex: 3 }}
      >
        <div className="collection-head">
          <p className="eyebrow anim-start" style={{ opacity: 0.7 }}>
            The Lineup
          </p>
          <h2
            className="anton mt-3 anim-start"
            style={{ fontSize: "clamp(44px, 8vw, 120px)" }}
          >
            Pick Your Crew
          </h2>
          <p
            className="text-sm mt-4 max-w-md anim-start"
            style={{ opacity: 0.7, lineHeight: 1.6 }}
          >
            Four characters, four attitudes. Each figurine is resin-printed,
            hand-finished and shipped in collector-grade packaging.
          </p>
        </div>
        ...
        <div className="collection-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6 mt-12 sm:mt-16">
          {CHARACTERS.map((character, index) => (
            ...
            >
              <div className="relative h-80 sm:h-96 flex items-end justify-center">
                <span
              ...
              <div
                className="px-5 sm:px-6 pt-1 pb-5 sm:pb-6 relative"
                style={{ zIndex: 3 }}
              >
                <p
